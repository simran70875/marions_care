...
import { customerSidebarMenu } from "../config/customerPanelMenu";
...
const CustomerSidebar: React.FC = () => {
  ...
  const currentMenu = useMemo(() => customerSidebarMenu, []);
          ...
          <li key={item.name}>
            {item.subItems ? (
              <>
                ...
                <div
                  className={`ml-8 mt-2 flex flex-col gap-2 transition-all duration-300 overflow-hidden ${openDropdowns[item.name] ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"}`}
                >
  ...
  return (
    ...
    >
      ...
      <div className="overflow-y-auto h-full">{renderMenu(currentMenu)}</div>
    </aside>
  );
};
