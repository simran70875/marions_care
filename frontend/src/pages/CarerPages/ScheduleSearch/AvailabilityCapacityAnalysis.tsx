...
const PRIMARY_BLUE = "#4C779C"; // Dark Blue/Gray-Blue for Search buttons
const ACCENT_BLUE = "#3B82F6"; // Standard blue for the Area Capacity button
...
interface CapacityRow {
  ...
  hrsMM: string; // Availability Used(%) Per Carer
  hrs: string; // Availability Used(%) Per Carer
  daysInRow: number;
  wed03: number | string;
  thu04: number | string;
  fri05: number | string;
...
// Helper function for conditional cell styling (re-used from previous component)
const renderCapacityCell = (
  params: GridRenderCellParams<any, number | string>
) => {
  const value =
  ...
  if (params.field === "hrs") {
    if (hrsValue > 25) {
      // Assuming red is for high capacity usage
...
const availabilityColumns: GridColDef<CapacityRow>[] = [
  ...
  { field: "status", headerName: "Status", width: 90, sortable: false }, // New Status column
...
const AvailabilityCapacityAnalysis: React.FC = () => {
    ...
    <Box className="bg-white min-h-screen">
      ...
      <Box className="px-4 py-3 bg-gray-100 border-b border-gray-300">
        ...
        <Typography variant="body2" className="text-sm text-gray-600">
          Report: Availability Used(%) Per Carer Grouped by Available,
          Scheduled, Capacity per day
        </Typography>
      </Box>
      ...
      >
        ...
        {/* Area Capacity Button (new component) */}
        <Button
          variant="contained"
          className="normal-case text-white shadow-md"
          sx={{
      ...
      {/* --- Legend --- */}
      {/* The screenshot shows a row of buttons/pills directly below the main search bar */}
      <Box className="p-4 flex flex-wrap items-center gap-4 border-b border-gray-200">
        ...
        <Box
          className="flex items-center gap-2 px-3 py-1 rounded-md"
          sx={{
            backgroundColor: GREEN_CAPACITY_THRESHOLD,
        ...
        <Box
          className="flex items-center gap-2 px-3 py-1 rounded-md"
          sx={{
            backgroundColor: BLUE_CAPACITY_THRESHOLD,
      ...
      <Box className="p-3 flex justify-between items-center border-b border-gray-200 bg-gray-50">
        {/* Left: Export Buttons (adjusted to match screenshot layout) */}
        <Box className="flex flex-wrap gap-2">
          {toolbarButtons.map((btn) => (
            ...
            >
              ...
              {/* Clean up labels to match screenshot */}
            </Button>
          ))}
        </Box>
...
export default AvailabilityCapacityAnalysis;
