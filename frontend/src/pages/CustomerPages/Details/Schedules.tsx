...
const ClientSchedulePage: React.FC = () => {
  ...
  const fetchShifts = async () => {
    ...
    const res = await call(shiftServices.getCustomerShifts(customerId, month));
    console.log("res ==> ", res);
  ...
  const fetchCarers = async () => {
    const res = await call(carerServices.getCarers());
    setCarers(res.data);
  };
  ...
  const handleSaveShift = async () => {
    ...
    console.log("customerId ==> ", customerId, editingShift);
    if (!customerId) return toast("Customer id is required");
    ...
    const payload = {
      customerId,
      carerId: editingShift.carerId || null,
      date: selectedDay,
      startTime,
      endTime,
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <select
                  ...
                  value={editingShift?.carerId || ""}
                  onChange={(e) =>
                    setEditingShift((prev) =>
                      prev
                        ? { ...prev, carerId: e.target.value || null }
                        : prev,
                    )
                  }
                >
