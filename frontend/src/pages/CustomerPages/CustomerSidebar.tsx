...
const CustomerSidebar: React.FC = () => {
  ...
  const currentMenu = useMemo(() => clientSubMenu, []);
          ...
          <li key={item.name}>
            {item.subItems ? (
              <>
                ...
                <div
                  className={`ml-8 mt-2 flex flex-col gap-2 transition-all duration-300 overflow-hidden ${openDropdowns[item.name]
                ...
                >
                  {item.subItems.map((sub) => (
                    <Link
                      key={sub.name}
                      to={sub.path || ""}
                      className={`text-sm pl-3 py-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-800 ${location.pathname === sub.path
                        ? "text-brand-600 font-medium" // Adjusted logic to match sub.path exactly
                        : "text-gray-600"
                        }`}
                    >
                      {sub.name}
